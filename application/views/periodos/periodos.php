				...
				<tr <?php if($periodo->deletado_em): echo 'class="danger"'; endif; ?>>
					...
					<td class="text-center">
						<?php if ( empty($periodo->deletado_em) ) : ?>
							...
							<a class="btn btn-danger glyphicon glyphicon-remove" title="Desativar" href="<?= site_url('periodo/deletar/'.$periodo->id)?>"></a>
						<?php else : ?>
							<a class="btn btn-success glyphicon glyphicon-check" title="Ativar" href="<?= site_url('periodo/ativar/'.$periodo->id)?>"></a>
						<?php endif; ?>
					</td>
				</tr>
			<?php } ?>
